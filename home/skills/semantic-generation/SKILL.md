...
description: |
  ...
  確立した用語だけで書ける短文、局所変数のリネームなど影響範囲の狭い命名。
---
...
本スキルは「対応表を先に独立提出し、本文はその写しとして書く」順序を強制する。
規律の基準は `~/.claude/rules/referent-before-label.md` が持つ。
...
- 保存先: 作業ディレクトリの `referent-table-<slug>.md`（成果物が `docs/` や `output/`
  配下なら同じディレクトリに置く）。
...
- 本スキルの本文には検証用の試験入力・期待回答を記載しない（検証の独立性を保つため。
  試験は別ディレクトリの fixture で管理する）。
- 誤用語リストとの照合を目的にしない。照合先は「その文書自身が宣言した対応表」である。
- dev-workflow スキルの設計フェーズ（`docs/` への要件定義書・設計書の作成）に着手する
  時は、設計書より先に本スキルの対応表を同じ `docs/` に保存する。
